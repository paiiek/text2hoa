# -*- coding: utf-8 -*-
# save as: eval_lastmile_v4cal.py
...
def U(s,c,el):
    return torch.stack([torch.cos(el)*c, torch.cos(el)*s, torch.sin(el)], -1)
...
# ---------- (1) train에서 "예측bin" 프로토타입 구축 ----------
@torch.no_grad()
def build_predbin_prototypes(model, state, Xtr, Ytr, bsz, device):
    # train 텍스트 인코딩 → 예측 → 예측bin별 실제 방향의 평균 유닛벡터/평균 el
    def enc_texts(xx):
        H=[]
        for i in range(0, len(xx), bsz):
            H.append(model.encode(xx[i:i+bsz], device).cpu())
    ...
    for i in range(0, len(Xtr), bsz):
        hh = Htr[i:i+bsz].to(device)
        yhat_list.append(head_forward_dynamic(hh, state, device).cpu())
    yhat = torch.cat(yhat_list, 0)  # [N,8]

    # 예측 bin
    a12_pred = torch.tensor([az12_from_sc(float(s), float(c)) for s,c in zip(yhat[:,0], yhat[:,1])])
    # 유닛벡터/엘리베이션
    u_true = U(Ytr[:,0], Ytr[:,1], Ytr[:,2])
    u_true = F.normalize(u_true, dim=-1)
    proto_u = torch.zeros(12,3); proto_el=torch.zeros(12); counts=torch.zeros(12)
    for k in range(12):
        m=(a12_pred==k)
        if m.any():
            uu = u_true[m]
            proto_u[k] = F.normalize(uu.mean(0, keepdim=True), dim=-1).squeeze(0)
            proto_el[k] = Ytr[m,2].mean()
            counts[k] = m.sum()
        else:
            # 빈 bin은 30° 중심을 fallback
            cen = math.radians(15 + 30*k)
            proto_u[k] = torch.tensor([math.cos(cen), math.sin(cen), 0.0])
            proto_el[k] = 0.0
    return proto_u.to(device), proto_el.to(device), counts

# ---------- (2) predbin 기준 스냅 ----------
def snap_to_predbin_protos(yh, proto_u, proto_el, alpha=0.25, gate_deg=10.0):
    # yh: [B,8]  (s,c,el만 보정)
    # 1) 현재 예측 bin
    a12_pred = torch.tensor([az12_from_sc(float(s), float(c)) for s,c in zip(yh[:,0].cpu(), yh[:,1].cpu())], device=yh.device)
    # 2) 유닛벡터 변환
    u_pred = U(yh[:,0], yh[:,1], yh[:,2]); u_pred = F.normalize(u_pred, dim=-1)
    u_ref  = proto_u[a12_pred]                     # [B,3]  (train에서 얻은 "그 bin을 예측했을 때의 실제 평균방향")
    # 3) 각도차
    dot = (u_pred*u_ref).sum(-1).clamp(-1+EPS,1-EPS)
    ddeg = torch.rad2deg(torch.acos(dot))
    msk = ddeg > gate_deg
    # 4) 블렌딩
    u_new = u_pred.clone()
    u_new[msk] = F.normalize((1-alpha)*u_pred[msk] + alpha*u_ref[msk], dim=-1)
    # el도 약하게 보정
    el_new = yh[:,2].clone()
    elp = proto_el[a12_pred]
    el_new[msk] = (1-0.5*alpha)*el_new[msk] + (0.5*alpha)*elp[msk]
    # 5) back to (s,c,el)
    x,y,z = u_new[:,0],u_new[:,1],u_new[:,2]
    az = torch.atan2(y, x)
    s = torch.sin(az); c = torch.cos(az)
    yh2 = yh.clone(); yh2[:,0]=s; yh2[:,1]=c; yh2[:,2]=el_new
    return yh2
...
def main():
    ...
    # pred-bin calibration
    ap.add_argument("--predbin_alpha", type=float, default=0.0)
    ...
    if args.use_knn:
        ...
        assert train_data==args.data, "KNN 일관성 위해 --train_data는 --data 동일 권장"
        Htr=None
    ...
    # (NEW) pred-bin 프로토타입 구축
    proto_u_pb=None; proto_el_pb=None
    if args.predbin_alpha > 0:
        with torch.no_grad():
            proto_u_pb, proto_el_pb, cnts = build_predbin_prototypes(m, state, Xtr, Ytr, args.bsz, dev)
        print("[predbin proto] counts:", [int(c) for c in cnts.cpu().tolist()])
    ...
    with torch.no_grad():
        ...
        def pct(hit, tot): return round(100.0*hit/max(1,tot),1)
        ...
        for i0 in range(0, len(Xte), args.bsz):
            ...
            # (NEW) pred-bin calibration
            if args.predbin_alpha > 0 and proto_u_pb is not None:
                yhat = snap_to_predbin_protos(yhat, proto_u_pb, proto_el_pb,
                                              alpha=args.predbin_alpha, gate_deg=args.predbin_gate)
            ...
            for j,t in enumerate(xs):
                ...
                if contains(t, ELEV_UP):  stats["gt"]["up"][1]+=1;   stats["gt"]["up"][0]+=int(gt_el>0)
                if contains(t, ELEV_DN):  stats["gt"]["down"][1]+=1; stats["gt"]["down"][0]+=int(gt_el<0)
                if contains(t, NEAR_W):   stats["gt"]["near"][1]+=1; stats["gt"]["near"][0]+=int(gt_d<1.5)
                if contains(t, FAR_W):    stats["gt"]["far"][1]+=1;  stats["gt"]["far"][0]+=int(gt_d>=3.0)

                pr_el=float(yh[j,2]); pr_d=float(yh[j,3])
                if contains(t, ELEV_UP):  stats["pred"]["up"][1]+=1;   stats["pred"]["up"][0]+=int(pr_el>0)
                if contains(t, ELEV_DN):  stats["pred"]["down"][1]+=1; stats["pred"]["down"][0]+=int(pr_el<0)
                if contains(t, NEAR_W):   stats["pred"]["near"][1]+=1; stats["pred"]["near"][0]+=int(pr_d<1.5)
                if contains(t, FAR_W):    stats["pred"]["far"][1]+=1;  stats["pred"]["far"][0]+=int(pr_d>=3.0)
        ...
        kw_report={
            "GT":   {"up%":pct(*tuple(stats["gt"]["up"])), "down%":pct(*tuple(stats["gt"]["down"])),
                     "near%":pct(*tuple(stats["gt"]["near"])), "far%":pct(*tuple(stats["gt"]["far"]))},
            "PRED": {"up%":pct(*tuple(stats["pred"]["up"])), "down%":pct(*tuple(stats["pred"]["down"])),
                     "near%":pct(*tuple(stats["pred"]["near"])), "far%":pct(*tuple(stats["pred"]["far"]))},
        }
        metrics={
            "N_test": len(Xte),
            ...
            "knn": {"k":args.knn_k,"thresh":args.knn_thresh,"alpha":args.knn_alpha,"temp":args.knn_temp,"angle_gate":args.knn_angle_gate},
            "predbin_calib": {"alpha": args.predbin_alpha, "gate": args.predbin_gate},
        ...
        print("[worst az12 3]:", [(i, float(means[i])) for i in range(12) if i in worst3])
        print("[lang AE]:", {k: round(v,2) for k,v in lang_metrics.items()})
        print("[kw consistency]:", kw_report)
        print("[pred-bin calib]:", metrics["predbin_calib"])
